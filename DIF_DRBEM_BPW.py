# -*- coding: utf-8 -*-
"""
Created on Sat Nov 16 08:05:56 2019

@author: Florin Albisoru

Description : problema de test din DRBEM
"""
...
#verifica matricea H sa fie completata cum trebuie            
...
C = -(1/a)*np.matmul(np.matmul(H,uh) - np.matmul(G,qh),Finv)
...
for t in range(T):
    system_freeterm = np.matmul(RM,up)
    system_freeterm = system_freeterm - np.matmul(LM[0:N+L,0:N],u[0:N])
    system_matrix[0:N+L,0:N] = -2*G[0:N+L,0:N]
    system_matrix[0:N+L,N:N+L] = LM[0:N+L,N:N+L]
